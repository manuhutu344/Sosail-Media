...
export default function Resetpassword() {
  ...
  const handleClick = async(e)=>{
    e.preventDefault();
    await fetch(`http://localhost:9000/user/reset/password?${code}` , {method:"PUT" , headers:{'Content-Type':"application/JSON"} , body:JSON.stringify({password:password})}).then((data)=>{
      alert("Password Anda Telah Di Rubah")
    })
  };
  return (
    <div style={{width:"100vw" , height:"100vh", display:'flex' , alignItems:"center" , justifyContent:"center"}}>
            <div style={{width:"25%" , padding:"20px" , margin:"auto" , borderRadius:"10px" , backgroundColor:"black"}}>
                <p style={{color:"white"}}>Masukan Password Anda</p>
